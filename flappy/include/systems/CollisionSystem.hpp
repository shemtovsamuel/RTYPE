...
namespace rtype
{
    namespace system
    {
        ...
        public:
             ...
             * @param entityManager
             */
            CollisionSystem(engine::ecs::EntityManager<rtype::config::ComponentCount, rtype::config::SystemCount> &entityManager) : _entityManager(entityManager), _assetManager(rtype::AssetManager::getInstance()), _entityTemplate(_entityManager, _assetManager)
            {
                setRequirements<
            ...
            void update(const float dt)
            {
                ...
                for (const auto &entity1 : getManagedEntities())
                {
                    auto [transform1, hitbox1] = _entityManager.getComponents<
                        engine::ecs::components::Transform,
                        engine::ecs::components::Hitbox>(entity1);
                    ...
                    for (const auto &entity2 : getManagedEntities())
                    {
                        ...
                        auto [transform2, hitbox2] = _entityManager.getComponents<
                            engine::ecs::components::Transform,
                            engine::ecs::components::Hitbox>(entity2);
                            ...
                            transform1.y + hitbox1.height > transform2.y)
                        {
                            ...
                            else if (hitbox1.tag == "Player" && hitbox2.tag == "Obstacle1Top")
                            {
                                auto &playerState = _entityManager.getComponent<rtype::ecs::components::State>(entity1);
                                playerState.toRemove = true;
                                _entityTemplate.gameOver();
                            }
                            else if (hitbox1.tag == "Player" && hitbox2.tag == "Obstacle1Bottom")
                            {
                                auto &playerState = _entityManager.getComponent<rtype::ecs::components::State>(entity1);
                                playerState.toRemove = true;
                                _entityTemplate.gameOver();
                            }
                            else if (hitbox1.tag == "Player" && hitbox2.tag == "Obstacle2Top")
                            {
                                auto &playerState = _entityManager.getComponent<rtype::ecs::components::State>(entity1);
                                playerState.toRemove = true;
                                _entityTemplate.gameOver();
                            }
                            else if (hitbox1.tag == "Player" && hitbox2.tag == "Obstacle2Bottom")
                            {
                                auto &playerState = _entityManager.getComponent<rtype::ecs::components::State>(entity1);
                                playerState.toRemove = true;
                                _entityTemplate.gameOver();
                            }
                            else if (hitbox1.tag == "Player" && hitbox2.tag == "Obstacle3Top")
                            {
                                auto &playerState = _entityManager.getComponent<rtype::ecs::components::State>(entity1);
                                playerState.toRemove = true;
                                _entityTemplate.gameOver();
                            }
                            else if (hitbox1.tag == "Player" && hitbox2.tag == "Obstacle3Bottom")
                            {
                                auto &playerState = _entityManager.getComponent<rtype::ecs::components::State>(entity1);
                                playerState.toRemove = true;
                                _entityTemplate.gameOver();
