...
bool rtype::debug::Debugger::hasComponents(rtype::Game& game, int id)
{
    if (game.getEntityManager().hasComponent<engine::ecs::components::Transform>(id))
        ...
        return true;
    else if (game.getEntityManager().hasComponent<rtype::ecs::components::PlayerId>(id))
        return true;
...
void rtype::debug::Debugger::drawEntityWindow(rtype::Game& game)
{
    ...
    if (_selectedEntity != -1) {
        ...
        if (game.getEntityManager().hasComponent<rtype::ecs::components::PlayerId>(_selectedEntity)) {
            if (ImGui::Selectable("PlayerId", _selectedComponent == (int)rtype::ecs::components::ComponentType::PlayerId))
            {
                _selectedComponent = (int)rtype::ecs::components::ComponentType::PlayerId;
            }
        }
